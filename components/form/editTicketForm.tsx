  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const finalInvalidInputs: TicketInputs = {
      ...
      employee:
        ...
        (editTicket.employeeId === null ||
          !employees
            .map((employee) => employee.legajo)
            .includes(editTicket.employeeId)),
      description:
        requiredInputs.description && editTicket.description.trim() === "",
      status:
        requiredInputs.status && !statusOptions.includes(editTicket.status),
      severity:
        requiredInputs.severity &&
        !["S1", "S2", "S3", "S4"].includes(editTicket.severity),
      client: false,
    };
    if (
