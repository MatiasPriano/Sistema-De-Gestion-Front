      ...
      employee:
        ...
        (newTicket.employeeId === null ||
          !employees
            ...
            .includes(newTicket.employeeId)),
      description:
        requiredInputs.description && newTicket.description.trim() === "",
      status:
        requiredInputs.status && !statusOptions.includes(newTicket.status),
      severity:
        requiredInputs.severity &&
        (newTicket.severity === null ||
          !["S1", "S2", "S3", "S4"].includes(newTicket.severity)),
      client:
        !disabledInputs.client &&
        requiredInputs.client &&
        (newTicket.clientId === null ||
          !clients.map((client) => client.id).includes(newTicket.clientId)),
    };
    if (
      ...
      <main className="grid gap-x-6 gap-y-4 sm:grid-cols-1 md:grid-cols-2 border bg-backgroundContainer border-border shadow-lg px-4 py-8 rounded-xl">
        ...
        <AutocompleteInput
          ...
          items={clients.map((client) => {
            return { id: client.id, name: client["razon social"] };
          })}
