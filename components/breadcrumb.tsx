  ...
  return (
    <Link href="/home/">
      <svg
        ...
        strokeWidth={1.5}
        stroke="currentColor"
        className="w-4 h-4 text-subtitle"
      >
        <path
          strokeLinecap="round"
  ...
  return (
    <div>
      {step.link !== null && (
        <Link
          href={step.link}
          className="truncate hover:underline text-subtitle font-medium"
          title={step.name}
        >
          {step.name}
        </Link>
      ...
      {step.link === null && (
        <div className="truncate text-subtitle font-medium">{step.name}</div>
      )}
    </div>
  );
}
      ...
      <div className="flex sm:hidden items-center space-x-2 relative">
        {steps.length > 1 && (
          <div className="flex items-center space-x-2">
            <div
              onClick={toggleFullBreadcrumb}
              className="cursor-pointer bg-backgroundAccent text-subtitle rounded-xl px-1"
            >
              ...
            </div>
            <BreadcrumbArrow />
        ...
        {showFullBreadcrumb && (
          <div className="absolute top-6 left-0 bg-backgroundAccent border rounded-md shadow-lg z-10">
            {steps.slice(0, -1).map((step, index) => (
              <div
                key={index}
                className="flex flex-col overflow-hidden items-start p-2 space-y-2"
              >
                <BreadcrumbLink step={step} />
              </div>
            ))}
